...
void CompressImage(string & infile, string & outfile, int Quant, trans Trans)
{
	...
	unsigned int imSize = img.width() * img.height() * img.spectrum();
	...
	CWavelet2D Wavelet(img.width(), img.height(), WAV_LEVELS, WAV_LEVELS - 4);
	Wavelet.SetWeight(Trans);

	if (Head.Color) {
		Wavelet.Transform(img.data(0,0,0,2), img.width(), Trans);
		Wavelet.CodeBand(&Codec, Quant ? Quants(Quant + SHIFT * 5) : 0, Quant ? Quants(Quant + SHIFT * 5 - 7) : 0);
		Wavelet.Transform(img.data(0,0,0,1), img.width(), Trans);
		Wavelet.CodeBand(&Codec, Quant ? Quants(Quant + SHIFT * 5 + C_Q_BOOST) : 0, Quant ? Quants(Quant + SHIFT * 5 - 7 + C_Q_BOOST) : 0);
		Wavelet.Transform(img.data(0,0,0,0), img.width(), Trans);
		Wavelet.CodeBand(&Codec, Quant ? Quants(Quant + SHIFT * 5 + C_Q_BOOST) : 0, Quant ? Quants(Quant + SHIFT * 5 - 7 + C_Q_BOOST) : 0);
	} else {
		Wavelet.Transform(img.data(0,0,0,0), img.width(), Trans);
		Wavelet.CodeBand(&Codec, Quant ? Quants(Quant + SHIFT * 5) : 0, Quant ? Quants(Quant + SHIFT * 5 - 7) : 0);
...
void DecompressImage(string & infile, string & outfile, bool Dither)
{
	...
	if (Head.Color) {
		Wavelet.TransformI(img.data() + width * heigth * 3, width, Trans);
		Wavelet.DecodeBand(&Codec);
		if (Head.Quant != 0)
			Wavelet.TSUQi(Quants(Head.Quant + SHIFT * 5 + C_Q_BOOST));
		Wavelet.TransformI(img.data() + width * heigth * 2, width, Trans);
		Wavelet.DecodeBand(&Codec);
		if (Head.Quant != 0)
			Wavelet.TSUQi(Quants(Head.Quant + SHIFT * 5 + C_Q_BOOST));
	}
	Wavelet.TransformI(img.data() + width * heigth, width, Trans);

	if (Head.Color == 0) {
		if (Head.Quant == 0)
			img += 128;
		else if (Dither) {
			dither(img.data(), img.width(), img.height());
			if (Head.Color) {
				dither(img.data() + width * heigth, img.width(), img.height());
				dither(img.data() + width * heigth * 2, img.width(), img.height());
			}
		} else
